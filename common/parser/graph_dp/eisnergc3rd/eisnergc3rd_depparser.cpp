	...
	void DepParser::decode() {
		...
		for (int i = 0; i < m_nSentenceLength; ++i) {
			...
			for (const auto & g : m_vecGrandsAsLeft[i]) {
				...
				item.l2r_solid_both[g] = ScoreAgenda();
				item.r2l_solid_both[g] = ScoreAgenda();
			}
			...
			for (int i = 0, max_i = m_nSentenceLength - d + 1; i < max_i; ++i) {
				...
				for (int k = i + 1; k < l; ++k) {
					...
					if (ritem.jux.find(i) != ritem.jux.end()) {
						...
						if (litem.l2r_solid_both.size() < m_vecGrandsAsLeft[i].size()) {
							for (const auto & agenda : litem.l2r_solid_both) {
								int g = agenda.first;
								if (g < i || g > l) {
									tscore l_base_score = l_base_jux + m_vecGrandChildScore[i][l][g] + grandBiSiblingArcScore(g, i, k, l);
									for (const auto & sws : agenda.second) {
										const int & j = sws->getSplit();
										item.updateL2RSolidBoth(g, k, j, l_base_score +
																			sws->getScore() +
							...
							for (const auto & g : m_vecGrandsAsLeft[i]) {
								if (litem.l2r_solid_both.find(g) != litem.l2r_solid_both.end()) {
									tscore l_base_score = l_base_jux + m_vecGrandChildScore[i][l][g] + grandBiSiblingArcScore(g, i, k, l);
									for (const auto & sws : litem.l2r_solid_both[g]) {
										const int & j = sws->getSplit();
										item.updateL2RSolidBoth(g, k, j, l_base_score +
																			sws->getScore() +
																			triSiblingArcScore(i, j == i ? -1 : j, k, l) +
					...
					if (ritem.l2r.find(i) != ritem.l2r.end()) {
						if (litem.l2r_solid_both.size() < m_vecGrandsAsLeft[i].size()) {
							for (auto && agenda : litem.l2r_solid_both) {
								int g = agenda.first;
								if (g < i || g > l) {
									item.updateL2R(g, k, agenda.second.bestItem().getScore() + ritem.l2r[i].getScore());
								}
							}
						}
						else {
							for (const auto & g : m_vecGrandsAsLeft[i]) {
								if (litem.l2r_solid_both.find(g) != litem.l2r_solid_both.end()) {
									item.updateL2R(g, k, litem.l2r_solid_both[g].bestItem().getScore() + ritem.l2r[i].getScore());
								}
							}
						}
					}
					...
					if (litem.jux.find(l) != litem.jux.end()) {
						...
						if (ritem.r2l_solid_both.size() < m_vecGrandsAsRight[l].size()) {
							for (const auto agenda : ritem.r2l_solid_both) {
								int g = agenda.first;
								if (g < i || g > l) {
									tscore r_base_score = r_base_jux + m_vecGrandChildScore[l][i][g] + grandBiSiblingArcScore(g, l, k, i);
									for (const auto & sws : agenda.second) {
										const int & j = sws->getSplit();
										item.updateR2LSolidBoth(g, k, j, r_base_score +
																			sws->getScore() +
							...
							for (const auto & g : m_vecGrandsAsRight[l]) {
								if (ritem.r2l_solid_both.find(g) != ritem.r2l_solid_both.end()) {
									tscore r_base_score = r_base_jux + m_vecGrandChildScore[l][i][g] + grandBiSiblingArcScore(g, l, k, i);
									for (const auto & sws : ritem.r2l_solid_both[g]) {
										const int & j = sws->getSplit();
										item.updateR2LSolidBoth(g, k, j, r_base_score +
																			sws->getScore() +
																			triSiblingArcScore(l, j == l ? -1 : j, k, i) +
					...
					if (litem.r2l.find(l) != litem.r2l.end()) {
						if (ritem.r2l_solid_both.size() < m_vecGrandsAsRight[l].size()) {
							for (auto && agenda : ritem.r2l_solid_both) {
								int g = agenda.first;
								if (g < i || g > l) {
									item.updateR2L(g, k, agenda.second.bestItem().getScore() + litem.r2l[l].getScore());
								}
							}
						}
						else {
							for (const auto & g : m_vecGrandsAsRight[l]) {
								if (ritem.r2l_solid_both.find(g) != ritem.r2l_solid_both.end()) {
									item.updateR2L(g, k, ritem.r2l_solid_both[g].bestItem().getScore() + litem.r2l[l].getScore());
								}
							}
						}
					}
				...
				for (const auto & g : m_vecGrandsAsLeft[i]) {
					if (m_lItems[d - 1][i + 1].r2l.find(i) != m_lItems[d - 1][i + 1].r2l.end()) {
						item.updateL2RSolidBoth(g, i, i, m_lItems[d - 1][i + 1].r2l[i].getScore() +
															l2r_arc_score +
					...
					if (item.l2r_solid_both.find(g) != item.l2r_solid_both.end()) {
						item.updateL2R(g, l, item.l2r_solid_both[g].bestItem().getScore());
					}
				}
				for (const auto & g : m_vecGrandsAsRight[l]) {
					if (m_lItems[d - 1][i].l2r.find(l) != m_lItems[d - 1][i].l2r.end()) {
						item.updateR2LSolidBoth(g, l, l, m_lItems[d - 1][i].l2r[l].getScore() +
															r2l_arc_score +
					...
					if (item.r2l_solid_both.find(g) != item.r2l_solid_both.end()) {
						item.updateR2L(g, i, item.r2l_solid_both[g].bestItem().getScore());
					}
				}
			}
		}
		...
		while (!stack.empty()) {
			...
			case L2R:
				...
				if (item.left < item.right - 1) {
					...
					stack.push(std::tuple<int, int, int, int>(g, s - item.left + 1, m_lItems[s - item.left + 1][item.left].l2r_solid_both[g].bestItem().getSplit(), item.left));
			...
			case R2L:
				...
				if (item.left < item.right - 1) {
					...
					stack.push(std::tuple<int, int, int, int>(g, item.right - s + 1, m_lItems[item.right - s + 1][s].r2l_solid_both[g].bestItem().getSplit(), s));
			...
			case L2R_SOLID_BOTH:
				...
				if (s == item.left) {
					...
					is = findInnerSplit(item.l2r_solid_both[g], s);
			...
			case R2L_SOLID_BOTH:
				...
				if (s == item.right) {
					...
					is = findInnerSplit(item.r2l_solid_both[g], s);
